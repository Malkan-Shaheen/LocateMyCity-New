...
const LeafletMap = ({ source, destination, distance }) => {
  ...
  const initializeMap = useCallback(() => {
    ...
    try {
      ...
      L.marker(midpoint, {
        icon: L.divIcon({
          html: `<div role="status" aria-live="polite" style="background: white; padding: 6px 10px; border-radius: 6px; border: 2px solid #4682B4; font-weight: 700; font-size: 12px; color: #000; box-shadow: 0 4px 8px rgba(0,0,0,0.3); white-space: nowrap; text-align: center; line-height: 1.2; display: inline-block; min-width: fit-content; max-width: 180px; overflow: hidden; text-overflow: ellipsis;">${distanceMiles} mi (${distance.toFixed(1)} km)<span class="sr-only"> Distance</span></div>`,
