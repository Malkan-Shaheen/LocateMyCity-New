...
import { getBeachLocations, getCityThemeMetadata } from '@/actions';
import BeachFAQSection from '../../components/BeachFAQSection';
import BeachCitiesNotable from '../../components/BeachCitiesNotable';
...
export default function BeachLocationsExplorer() {
    ...
    const loadData = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setMetadata(meta);
          setAnnouncement(`Loaded ${data.length} locations with "Beach" in their name`);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (allLocations.length > 0) {
      setDisplayedLocations(allLocations.slice(0, visibleCount));
    }
  }, [visibleCount, allLocations]);

  const handleLoadMore = () => {
    setVisibleCount((prev) => Math.min(prev + LOCATIONS_PER_LOAD, allLocations.length));
  };
  ...
  const locationsForSelectedState = useMemo(() => {
    return selectedUSState ? allLocations.filter((l) => l.state === selectedUSState) : [];
  }, [allLocations, selectedUSState]);
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        {/* City Theme Links */}
        <CityThemeLinks currentTheme="beach" />
        ...
        <section className="interactive-map-section">
          <div className="content-container">
            ...
            <div ref={mapContainerRef}>
              {mapVisible ? (
                <MapWithNoSSR locations={allLocations} />
              ) : (
                <div className="loading-indicator">
                  <div className="loading-spinner"></div>
        ...
        <section className="all-locations-section">
          <div className="content-container">
            ...
            <div className="text-center mb-3 font-medium">
              Showing {displayedLocations.length} of {allLocations.length} locations
            </div>
            ...
            {visibleCount < allLocations.length && (
              <div className="text-center mt-6">
                <button className="view-map-btn load-more-btn" onClick={handleLoadMore}>
                  Load More — Showing {displayedLocations.length} of {allLocations.length}
                </button>
              </div>
            )}
          </div>
