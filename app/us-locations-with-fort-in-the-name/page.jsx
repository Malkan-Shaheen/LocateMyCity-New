...
import { getFortLocations, getCityThemeMetadata } from '@/actions';
import FortFAQSection from '../../components/FortFAQSection';
import FortCitiesNotable from '../../components/FortCitiesNotable';
...
export default function FortLocationsExplorer() {
  const [allFortLocations, setAllFortLocations] = useState([]);
    ...
    const loadData = async () => {
      try {
        ...
        const [data, meta] = await Promise.all([
          getFortLocations(),
          getCityThemeMetadata('fort')
        ]);

        if (isMounted) {
          setAllFortLocations(data);
          setDisplayedLocations(data.slice(0, LOCATIONS_PER_LOAD));
          setMetadata(meta);
          setAnnouncement(`Loaded ${data.length} locations with "Fort" in their name`);
        }
      } catch (error) {
  ...
  // Load More
  useEffect(() => {
    if (allFortLocations.length > 0) {
      setDisplayedLocations(allFortLocations.slice(0, visibleCount));
    }
  }, [visibleCount, allFortLocations]);

  const handleLoadMore = () => {
    setVisibleCount((prev) => Math.min(prev + LOCATIONS_PER_LOAD, allFortLocations.length));
  };

  // Analytics helpers
  const commonLocationNames = useMemo(() => {
    const freq = {};
    allFortLocations.forEach((loc) => {
  ...
  const uniqueUSStates = useMemo(
    () => [...new Set(allFortLocations.map((l) => l.state))].sort(),
    [allFortLocations]
  );

  const locationsForSelectedState = useMemo(() => {
    return selectedUSState ? allFortLocations.filter((l) => l.state === selectedUSState) : [];
  }, [allFortLocations, selectedUSState]);
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        <section className="py-16 notable-cities-explainer">
          <div className="content-container max-w-3xl mx-auto text-center leading-relaxed">
            <h2 className="explainer-title mb-6">
              U.S. Cities and Towns With "Fort" in Their Name
            </h2>

            <p className="explainer-paragraph">
              Why do so many cities and towns include the word "Fort" in their name? The answer is rooted in military history and early settlement patterns. Forts were often among the first permanent structures built in a region, serving as defensive outposts, trading hubs, or strategic checkpoints during periods of expansion and conflict.
            </p>

            <p className="explainer-paragraph">
              In many cases, civilian settlements formed around these forts, benefiting from protection, commerce, and access to transportation routes. As military use declined or shifted elsewhere, the surrounding communities continued to grow, retaining the word "Fort" as a lasting marker of their origins.
            </p>

            <p className="explainer-paragraph">
              Today, cities and towns with "Fort" in their name are found throughout the United States. Some have grown into major urban centers, while others remain smaller communities closely tied to their historical role. Together, they highlight how military infrastructure influenced settlement, migration, and place naming across the country.
            </p>

            <p className="explainer-paragraph">
              This page provides a complete, searchable list of U.S. locations containing the word "Fort," along with interactive maps, state filtering, and geographic insights to help explore these historically significant places.
            </p>
          </div>
        </section>
        ...
        <section className="py-20" style={{ 
          ...
          <div className="content-container max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="section-title text-center mb-6" style={{ 
              ...
              Why So Many Places Use "Fort" in the Name
            </h2>
            ...
            <div className="flex justify-center mb-12" style={{ marginBottom: '3rem' }}>
              <p className="text-center text-gray-700 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                The word "Fort" is commonly used in place names to mark:
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              <div className="why-card bg-white rounded-3xl border-2 border-blue-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Military outposts established during territorial expansion
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-indigo-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Defensive positions near borders, rivers, or trade routes
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-purple-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Settlements that formed around army installations
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-pink-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Strategic locations that later evolved into civilian towns
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-center mt-8" style={{ paddingTop: '2rem' }}>
              <p className="text-center text-gray-800 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                In many regions, the original fort no longer exists, but the name remains as a reminder of the area's role in early national history.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* City Theme Links */}
        <CityThemeLinks currentTheme="fort" />
        ...
        <section className="interactive-map-section">
          <div className="content-container">
            ...
            <div ref={mapContainerRef}>
              {mapVisible ? (
                <MapWithNoSSR locations={allFortLocations} />
              ) : (
                <div className="loading-indicator">
                  <div className="loading-spinner"></div>
        ...
        <section className="all-locations-section">
          <div className="content-container">
            ...
            <div className="text-center mb-3 font-medium">
              Showing {displayedLocations.length} of {allFortLocations.length} locations
            </div>
            ...
            {visibleCount < allFortLocations.length && (
              <div className="text-center mt-6">
                <button className="view-map-btn load-more-btn" onClick={handleLoadMore}>
                  Load More — Showing {displayedLocations.length} of {allFortLocations.length}
                </button>
              </div>
            )}
          </div>
