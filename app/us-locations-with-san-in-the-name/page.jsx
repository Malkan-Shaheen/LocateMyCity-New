...
import { getSanLocations, getCityThemeMetadata } from '@/actions';
import SanFAQSection from '../../components/SanFAQSection';
import SanCitiesNotable from '../../components/SanCitiesNotable';
...
export default function SanLocationsExplorer() {
  const [allSanLocations, setAllSanLocations] = useState([]);
    ...
    const loadData = async () => {
      try {
        ...
        const [data, meta] = await Promise.all([
          getSanLocations(),
          getCityThemeMetadata('san')
        ]);

        if (isMounted) {
          setAllSanLocations(data);
          setDisplayedLocations(data.slice(0, LOCATIONS_PER_LOAD));
          setMetadata(meta);
          setAnnouncement(`Loaded ${data.length} locations with "San" in their name`);
        }
      } catch (error) {
  ...
  // Load More
  useEffect(() => {
    if (allSanLocations.length > 0) {
      setDisplayedLocations(allSanLocations.slice(0, visibleCount));
    }
  }, [visibleCount, allSanLocations]);

  const handleLoadMore = () => {
    setVisibleCount((prev) => Math.min(prev + LOCATIONS_PER_LOAD, allSanLocations.length));
  };

  // Analytics helpers
  const commonLocationNames = useMemo(() => {
    const freq = {};
    allSanLocations.forEach((loc) => {
  ...
  const uniqueUSStates = useMemo(
    () => [...new Set(allSanLocations.map((l) => l.state))].sort(),
    [allSanLocations]
  );

  const locationsForSelectedState = useMemo(() => {
    return selectedUSState ? allSanLocations.filter((l) => l.state === selectedUSState) : [];
  }, [allSanLocations, selectedUSState]);
...
<section className="py-16 notable-cities-explainer">
  <div className="content-container max-w-3xl mx-auto text-center leading-relaxed">
    <h2 className="explainer-title mb-6">
              U.S. Cities and Towns With "San" in Their Name
    </h2>

    <p className="explainer-paragraph">
              Why do so many cities and towns include the word "San" in their name? The answer lies in Spanish language and colonial influence, particularly in regions once governed or shaped by Spain. Derived from the Spanish word for "Saint," "San" was commonly used to honor religious figures, Catholic missions, and patron saints during early settlement.
    </p>

    <p className="explainer-paragraph">
              Many locations with "San" in their name emerged around Spanish missions, presidios, or important trade routes, especially throughout the Southwest and along the Pacific coast. As these settlements expanded into modern cities, their original names were preserved, becoming permanent reminders of local geography, faith, and cultural heritage.
    </p>

    <p className="explainer-paragraph">
              Today, cities with "San" in their name are most heavily concentrated in states such as California, Texas, Arizona, and New Mexico. Some have grown into major metropolitan centers with global recognition, while others remain smaller communities deeply connected to their historical roots. Together, these places reflect the lasting imprint of Spanish exploration and settlement on American place names.
            </p>

            <p className="explainer-paragraph">
              This page provides a complete, searchable list of all U.S. locations containing the word "San", along with interactive maps, state filtering, and geographic insights to help explore these culturally significant places.
            </p>
          </div>
        </section>
        ...
        <section className="py-20" style={{ 
          ...
          <div className="content-container max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="section-title text-center mb-6" style={{ 
              ...
              Why So Many Places Use "San" in the Name
            </h2>
            ...
            <div className="flex justify-center mb-12" style={{ marginBottom: '3rem' }}>
              <p className="text-center text-gray-700 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                The word "San" appears in place names primarily to mark:
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              <div className="why-card bg-white rounded-3xl border-2 border-blue-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Spanish mission settlements named after Catholic saints
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-indigo-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Locations established during Spanish colonial rule
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-purple-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Religious or cultural dedication tied to local patron saints
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-pink-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Early towns that retained their original Spanish identity
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-center mt-8" style={{ paddingTop: '2rem' }}>
              <p className="text-center text-gray-800 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                In many regions, "San" names remain unchanged as a reflection of historical continuity, language, and cultural heritage.
              </p>
            </div>
  </div>
</section>
        ...
        {/* City Theme Links */}
        <CityThemeLinks currentTheme="san" />
        ...
        <section className="interactive-map-section">
          <div className="content-container">
            ...
            <div ref={mapContainerRef}>
              {mapVisible ? (
                <MapWithNoSSR locations={allSanLocations} />
              ) : (
                <div className="loading-indicator">
                  <div className="loading-spinner"></div>
        ...
        <section className="all-locations-section">
          <div className="content-container">
            ...
            <div className="text-center mb-3 font-medium">
              Showing {displayedLocations.length} of {allSanLocations.length} locations
            </div>
            ...
            {visibleCount < allSanLocations.length && (
              <div className="text-center mt-6">
                <button className="view-map-btn load-more-btn" onClick={handleLoadMore}>
                  Load More — Showing {displayedLocations.length} of {allSanLocations.length}
                </button>
              </div>
            )}
          </div>
