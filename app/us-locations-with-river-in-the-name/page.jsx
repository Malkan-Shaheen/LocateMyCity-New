...
import { getRiverLocations, getCityThemeMetadata } from '@/actions';
import RiverFAQSection from '../../components/RiverFAQSection';
import RiverCitiesNotable from '../../components/RiverCitiesNotable';
...
export default function RiverLocationsExplorer() {
  const [allRiverLocations, setAllRiverLocations] = useState([]);
    ...
    const loadData = async () => {
      try {
        ...
        const [data, meta] = await Promise.all([
          getRiverLocations(),
          getCityThemeMetadata('river')
        ]);

        if (isMounted) {
          setAllRiverLocations(data);
          setDisplayedLocations(data.slice(0, LOCATIONS_PER_LOAD));
          setMetadata(meta);
          setAnnouncement(`Loaded ${data.length} locations with "River" in their name`);
        }
      } catch (error) {
  ...
  // Load More
  useEffect(() => {
    if (allRiverLocations.length > 0) {
      setDisplayedLocations(allRiverLocations.slice(0, visibleCount));
    }
  }, [visibleCount, allRiverLocations]);

  const handleLoadMore = () => {
    setVisibleCount((prev) => Math.min(prev + LOCATIONS_PER_LOAD, allRiverLocations.length));
  };

  // Analytics helpers
  const commonLocationNames = useMemo(() => {
    const freq = {};
    allRiverLocations.forEach((loc) => {
  ...
  const uniqueUSStates = useMemo(
    () => [...new Set(allRiverLocations.map((l) => l.state))].sort(),
    [allRiverLocations]
  );

  const locationsForSelectedState = useMemo(() => {
    return selectedUSState ? allRiverLocations.filter((l) => l.state === selectedUSState) : [];
  }, [allRiverLocations, selectedUSState]);
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        <section className="py-16 notable-cities-explainer">
          <div className="content-container max-w-3xl mx-auto text-center leading-relaxed">
            <h2 className="explainer-title mb-6">
              U.S. Cities and Towns With "River" in Their Name
            </h2>

            <p className="explainer-paragraph">
              Why do so many cities and towns include the word "River" in their name? The answer is closely tied to geography. Rivers played a critical role in early settlement, providing fresh water, transportation routes, fertile land, and access to trade. Many communities formed along riverbanks where movement and resources naturally converged.
            </p>

            <p className="explainer-paragraph">
              In many cases, towns with "River" in their name developed beside major waterways, becoming centers for shipping, milling, logging, or regional commerce. Even as these settlements expanded and modernized, the river remained a defining feature, permanently shaping the town's identity and name.
            </p>

            <p className="explainer-paragraph">
              Today, cities with "River" in their name can be found in every region of the country. Some are mid-sized cities influenced by industrial growth, while others are smaller towns closely tied to surrounding ecosystems. Together, they reveal how rivers shaped settlement patterns, local economies, and place-naming traditions across the United States.
            </p>

            <p className="explainer-paragraph">
              This page offers a complete, searchable list of U.S. locations containing the word "River," with interactive maps, state-based filtering, and geographic insights to help explore these water-connected communities.
            </p>
          </div>
        </section>
        ...
        <section className="py-20" style={{ 
          ...
          <div className="content-container max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="section-title text-center mb-6" style={{ 
              ...
              Why So Many Places Use "River" in the Name
            </h2>
            ...
            <div className="flex justify-center mb-12" style={{ marginBottom: '3rem' }}>
              <p className="text-center text-gray-700 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                The word "River" is commonly used in place names to mark:
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              <div className="why-card bg-white rounded-3xl border-2 border-blue-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Settlements established along major waterways
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-indigo-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Towns dependent on river-based trade or transport
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-purple-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Locations shaped by flooding, navigation, or river crossings
                </p>
              </div>
              ...
              <div className="why-card bg-white rounded-3xl border-2 border-pink-200 shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-3" style={{
                ...
                <p className="text-gray-700 text-center leading-relaxed" style={{ fontSize: '1rem', lineHeight: '1.7' }}>
                  Geographic identifiers distinguishing nearby communities
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-center mt-8" style={{ paddingTop: '2rem' }}>
              <p className="text-center text-gray-800 text-xl leading-relaxed max-w-3xl" style={{ 
                ...
                In many regions, the river remains central to the town's economy, environment, and historical development.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* City Theme Links */}
        <CityThemeLinks currentTheme="river" />
        ...
        <section className="interactive-map-section">
          <div className="content-container">
            ...
            <div ref={mapContainerRef}>
              {mapVisible ? (
                <MapWithNoSSR locations={allRiverLocations} />
              ) : (
                <div className="loading-indicator">
                  <div className="loading-spinner"></div>
        ...
        <section className="all-locations-section">
          <div className="content-container">
            ...
            <div className="text-center mb-3 font-medium">
              Showing {displayedLocations.length} of {allRiverLocations.length} locations
            </div>
            ...
            {visibleCount < allRiverLocations.length && (
              <div className="text-center mt-6">
                <button className="view-map-btn load-more-btn" onClick={handleLoadMore}>
                  Load More — Showing {displayedLocations.length} of {allRiverLocations.length}
                </button>
              </div>
            )}
          </div>
